...
Shader "Unlit/MoureOrb"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half2 barrelDistortion(half2 coord, half spherical, half scale)
			{
				half2 h = coord.xy - half2(0.5, 0.5);
				half r2 = dot(h, h);
				half f = 1.0 + r2 * (spherical * sqrt(r2));
			...
			fixed4 frag(v2f IN) : COLOR
			{
				...
				col += _Params01.w * col * _LineCol * tex2D(_BgTex01, PanUV_02).a * LineMask;
